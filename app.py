...
import soundfile as sf  # 에러 방지를 위한 새로운 라이브러리

st.set_page_config(layout="wide", page_title="GarageLight DAW")
st.title("🎼 GarageLight: Optical Synth DAW (No-Error Version)")
...
if uploaded_file:
    try:
        ...
        # 오디오 캔버스 생성 (float32로 고정)
        audio_len = int(sample_rate * (total_frames / fps)) + sample_rate
        master_audio = np.zeros((audio_len, 2), dtype=np.float32)
        ...
        for i in range(total_frames):
            ...
            for idx, cnt in enumerate(sorted_cnts):
                ...
                if end_idx < audio_len:
                    master_audio[start_idx:end_idx, 0] += tone * pan_l
                    master_audio[start_idx:end_idx, 1] += tone * pan_r
        ...
        # --- [에러 해결의 핵심] ---
        # 1. 피크 노멀라이징 (절대값 1.0을 넘지 않게 함)
        max_val = np.max(np.abs(master_audio))
        if max_val > 0:
            master_audio /= max_val
        ...
        with col1:
            ...
            # soundfile을 사용하여 메모리 내에서 WAV 생성 (가장 안전한 방식)
            buffer = io.BytesIO()
            sf.write(buffer, master_audio, sample_rate, format='WAV', subtype='PCM_16')
            st.audio(buffer.getvalue())
            st.download_button("💾 Download WAV", buffer.getvalue(), "bus_music.wav")

        with col2:
            st.header("📊 Timeline")
            for idx in range(6):
                if tracks_visual[idx]:
                    fig = go.Figure(go.Scatter(x=[v['t'] for v in tracks_visual[idx]], y=[v['f'] for v in tracks_visual[idx]], mode='lines', line_color='#00d1ff'))
